...
class NormDegDataset(Dataset):
    def __init__(self, h5_path, noise_mv=5.0, split="train", frac=0.8, seed=42):
        ...
        if noise_mv > 0:
            ...
            noise = rng.normal(0, noise_mv / 1000, V_norm.shape).astype(np.float32)
            V_norm = np.clip(V_norm + noise, -0.1, 1.1)
        ...
        self.V = torch.tensor(V_norm, dtype=torch.float32)
        self.cr = torch.tensor(cr.reshape(-1, 1), dtype=torch.float32)
        rng2 = np.random.RandomState(seed)
        idx = rng2.permutation(len(V))
        nt = int(len(V) * frac)
        if split == "train":
            self.idx = sorted(idx[:nt])
        else:
            self.idx = sorted(idx[nt:])
...
class BayesianNormModel(nn.Module):
    def __init__(self):
        super().__init__()
...
def train():
    ...
    h5 = "data/fullfield/fullfield_lfp_degradation.h5"
    train_ds = NormDegDataset(h5, noise_mv=5.0, split="train")
    val_ds = NormDegDataset(h5, noise_mv=5.0, split="val")
    train_loader = DataLoader(train_ds, batch_size=128, shuffle=True, num_workers=0)
    val_loader = DataLoader(val_ds, batch_size=128, shuffle=False, num_workers=0)

    model = BayesianNormModel().to(device)
        ...
        if ep % 50 == 0:
            ...
            with torch.no_grad():
                for V, cr, params in val_loader:
                    ...
                    su.append(
                        s[:, [i for i in range(7) if i not in IDENT_IDX]]
                        .mean()
                        .item()
                    )
            vloss /= nb
            logger.info(
                ...
                np.mean(si),
                np.mean(su),
                np.mean(su) / np.mean(si),
            )
            if vloss < best_val:
                best_val = vloss
                out_dir = Path("outputs/bayesian/calibrated_norm")
...
def evaluate_experimental(model, p_mean, p_std):
        ...
        for key in sorted(f["cells"].keys()):
            ...
            with torch.no_grad():
                for si in sample_idx:
                    vt = torch.tensor(V_n[si : si + 1], dtype=torch.float32).to(device)
                    mu, lv = model(vt, cr_t)
    ...
    results_summary["id_sharp"] = float(id_s)
    results_summary["un_sharp"] = float(un_s)
    results_summary["ratio"] = float(un_s / id_s)
    results_summary["n_cells"] = len(all_res)

    out_dir = Path("outputs/bayesian/calibrated_norm_exp")
    out_dir.mkdir(parents=True, exist_ok=True)
    np.savez(
