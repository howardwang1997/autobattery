...
We present a rigorous Fisher Information analysis of the single particle model (SPM) under degradation, proving that the voltage response has effective rank $\leq 3$ across five lithium-ion chemistries (NMC811, NCA, LFP, LFP\textsubscript{v2}, LCO), implying that at most three of seven degradation mechanisms can be simultaneously identified from constant-current discharge data.
Mutual information estimation yields a 28.7$\times$ separation ratio between identifiable parameters (D$_n$, SEI, LAM$_{\mathrm{neg}}$) and unidentifiable ones (D$_p$, $t^+$, LAM$_{\mathrm{pos}}$, R$_{\mathrm{mult}}$).
A Fisher-guided signature selection using only three parameters recovers 99.4\% of the full seven-parameter model accuracy ($R^2 = 0.985$ vs.\ $0.990$), outperforming random subsets by 1.1$\sigma$.
...
Multi-rate Fisher analysis gains +1 rank (3$\to$4) in both LFP and LIB chemistries, providing a quantitative justification for multi-C-rate diagnostic protocols.
...
\begin{enumerate}
\item We prove, via Fisher Information Matrix (FIM) analysis across five lithium-ion chemistries, that the SPM voltage response has effective rank $\leq 3$ for seven degradation parameters, implying a fundamental limit on simultaneous parameter identification (Section~\ref{sec:theory}).
...
\begin{table}[htbp]
\centering
\caption{Parameter identifiability classification from Fisher Information analysis (LFP, log-standardised). CRLB is the Cram\'er--Rao lower bound on estimation variance.}
\label{tab:groups}
\begin{tabular}{lccc}
\toprule
Parameter & CRLB & Group & $\Delta R^2$ (LOO) \\
\midrule
D$_n$ & finite, tight & ID & 0.0437 \\
SEI & finite, tight & ID & 0.0021 \\
LAM$_{\mathrm{neg}}$ & finite, tight & ID & 0.0066 \\
\midrule
D$_p$ & infinite & UN & 0.0002 \\
$t^+$ & infinite & UN & 0.0006 \\
LAM$_{\mathrm{pos}}$ & infinite & UN & 0.0005 \\
R$_{\mathrm{mult}}$ & infinite & UN & 0.0023 \\
\bottomrule
\end{tabular}
\end{table}
...
The Fisher-guided 3-parameter subset \{D$_n$, SEI, LAM$_{\mathrm{neg}}$\} achieves $R^2 = 0.985$, recovering 99.4\% of the full 7-parameter model accuracy ($R^2 = 0.990$).
Random 3-parameter subsets achieve $R^2 = 0.861 \pm 0.115$, placing the Fisher-guided result 1.1$\sigma$ above the random mean.
...
The forward selection order is: D$_n$ ($R^2 = 0.538$) $\to$ LAM$_{\mathrm{neg}}$ ($R^2 = 0.968$) $\to$ R$_{\mathrm{mult}}$ ($R^2 = 0.985$) $\to$ SEI $\to$ LAM$_{\mathrm{pos}} \to t^+ \to$ D$_p$.
Notably, the first two selected parameters (D$_n$, LAM$_{\mathrm{neg}}$) match the Fisher ID group, providing independent validation.

The leave-one-out analysis (Table~\ref{tab:groups}, right column) confirms D$_n$ as the most critical parameter ($\Delta R^2 = 0.044$), followed by LAM$_{\mathrm{neg}}$ ($\Delta R^2 = 0.007$).
The four Fisher-UN parameters have $\Delta R^2 < 0.003$.
...
The parameters D$_p$, $t^+$, and LAM$_{\mathrm{pos}}$ have small but non-zero Jacobian entries that accumulate to a detectable signal over the full voltage curve when the model is evaluated at many time points.
...
The Fisher-guided 3-parameter model (D$_n$, SEI, LAM$_{\mathrm{neg}}$) achieves 99.4\% of full model accuracy, and the SVD upper bound at $k = 3$ achieves $R^2 = 0.998$, confirming that the information bottleneck is in the data, not the model.
...
\item The Fisher analysis assumes the SPM, which neglects electrode-level spatial gradients.
The DFN model yields higher effective rank (3.79, numerical rank 4), but is computationally expensive for fullfield analysis.
\item The GP surrogate's near-perfect fit ($R^2 = 0.999$) may overestimate profile likelihood sharpness for parameters with very small but real sensitivity.
\item The MIT Severson dataset lacks ground-truth degradation mechanism labels, preventing direct validation of which parameters are active.
\end{enumerate}
...
We have demonstrated, through Fisher Information analysis across five lithium-ion chemistries, that constant-current voltage data provides at most 3 linearly independent pieces of information about 7 degradation parameters ($\eta$-rank $= 3$ at $\eta = 10^{-3}$).
This low-rank structure is universal, confirmed for both LFP (1{,}200 simulations) and LIB (1{,}888 simulations) chemistries.

Mutual information analysis confirms the Fisher classification with a 28.7$\times$ separation ratio between identifiable (D$_n$, SEI, LAM$_{\mathrm{neg}}$) and unidentifiable parameter groups.
Fisher-guided model reduction to 3 parameters recovers 99.4\% of full model accuracy, while multi-rate protocols gain one additional identifiable dimension (rank 3 $\to$ 4).

Validation against 138 real LFP cells from the MIT Severson dataset reveals that real-world degradation is even more degenerate than theory predicts (median effective rank $= 1.22$), indicating single-mechanism dominance.
Profile likelihood analysis with a GP surrogate identifies R$_{\mathrm{mult}}$ as the sole structurally unidentifiable parameter, while cross-method disagreement for other parameters underscores the method-dependent nature of identifiability conclusions.
