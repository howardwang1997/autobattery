...
An MC-Dropout Bayesian neural network achieves $R^2 = 0.968$ for predicting capacity fade percentage from $\Delta V$ features (Table~\ref{tab:bnn}, Fig.~\ref{fig:bnn}), confirming that voltage evolution strongly encodes degradation state despite the low-rank structure.
...
\begin{figure}[htbp]
\centering
\includegraphics[width=\linewidth]{figures/fig3_bnn.png}
\caption{BNN prediction on MIT Severson dataset. Left: fade percentage from $\Delta V$ ($R^2 = 0.968$). Right: log(cycle life) from $\Delta V$ ($R^2 = 0.827$). Error bars show MC-Dropout uncertainty.}
\label{fig:bnn}
\end{figure}
...
Correlation analysis of the Jacobian columns (Fig.~\ref{fig:corr}) reveals that SEI, $t^+$, and R$_{\mathrm{mult}}$ produce \emph{nearly identical} voltage changes, with pairwise correlations of 0.999--1.000.
...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.85\linewidth]{figures/fig9_correlation.png}
\caption{Jacobian column correlation matrix. SEI, $t^+$, and R$_{\mathrm{mult}}$ form a degenerate triplet with pairwise correlations $> 0.999$. All other parameter pairs are uncorrelated ($|r| \approx 0$), reflecting the block structure of the sensitivity.}
\label{fig:corr}
\end{figure}
...
A direct test of model reduction confirms that the Fisher-unidentifiable parameters are redundant for voltage prediction (Fig.~\ref{fig:reduction}).
...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.85\linewidth]{figures/fig10_model_reduction.png}
\caption{Model reduction comparison. Fisher-ID 3-param model achieves identical V(t) prediction ($R^2 = 0.726$) as the full 7-param model, confirming that unidentifiable parameters are redundant for voltage prediction.}
\label{fig:reduction}
\end{figure}
...
Systematic noise sweep from $\sigma = 0$ to 100~mV shows that the ID/UN separation in recoverability is robust to realistic measurement noise (Fig.~\ref{fig:noise}).
...
\begin{figure}[htbp]
\centering
\includegraphics[width=\linewidth]{figures/fig11_noise.png}
\caption{Noise robustness of identifiability classification. Left: ID group mean $R^2$ (blue) remains well above UN group (red) across all noise levels. Right: separation ratio stays above 10$\times$ up to $\sigma = 20$~mV, well within typical battery tester precision. Green shading indicates typical noise range.}
\label{fig:noise}
\end{figure}
...
Concatenating voltage curves from multiple C-rates (0.5C, 1C, 2C) into a single feature vector significantly improves parameter recovery for the Fisher-ID group (Fig.~\ref{fig:multirate}): SEI recovery improves from $R^2 = 0.63$ to $R^2 = 0.98$, and LAM$_{\mathrm{neg}}$ from $R^2 = 0.19$ to $R^2 = 0.79$.
...
\begin{figure}[htbp]
\centering
\includegraphics[width=\linewidth]{figures/fig12_multirate.png}
\caption{Multi-rate parameter recovery. Blue/red bars indicate Fisher ID/UN classification. Multi-rate concatenation (solid bars) substantially improves recovery for ID parameters (SEI: 0.63 $\to$ 0.98, LAM$_{\mathrm{neg}}$: 0.19 $\to$ 0.79) while UN parameters remain near zero.}
\label{fig:multirate}
\end{figure}
...
The signature ablation experiment on the LIB dataset (1{,}888 simulations, NMC811/graphite chemistry) yields qualitatively different results from LFP (Fig.~\ref{fig:xchem}).
...
\begin{figure}[htbp]
\centering
\includegraphics[width=\linewidth]{figures/fig13_cross_chemistry.png}
\caption{Cross-chemistry leave-one-out ablation. Left: LFP parameters coloured by Fisher ID/UN classification (SEI dominates). Right: LIB parameters (exchange-current densities dominate). Despite different chemistry-specific rankings, both datasets exhibit rank $= 3$ at $\eta = 10^{-3}$.}
\label{fig:xchem}
\end{figure}
