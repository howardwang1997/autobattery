...
**Param order** (LFP fullfield h5 file): `["D_n", "D_p", "t+", "SEI", "LAM_neg", "LAM_pos", "R_mult"]`
⚠️ **Bug fixed**: scripts previously used wrong order `["SEI","LAM_neg","LAM_pos","D_n","D_p","t+","R_mult"]`. All scripts corrected as of commit fix-param-names.
...
- CRLB: SEI and D_n finite+tight, D_p/t+/LAM_pos/R_mult all inf
...
- **Fisher ID group**: SEI, D_n, LAM_neg
...
**Forward selection order**: SEI → D_p → R_mult → D_n → t+ → LAM_pos → LAM_neg
- First 1 (SEI) matches Fisher ID group
- Fisher-guided 3 params (SEI, D_n, LAM_neg) capture 98.0% of full 7-param performance

**Leave-one-out importance (ΔR²):**
| Param | ΔR² when removed | Category |
|-------|-----------------|----------|
| SEI | **0.0437** | ID (critical) |
| D_p | **0.0066** | UN |
| R_mult | 0.0023 | UN |
| D_n | 0.0021 | ID |
| LAM_pos | 0.0006 | UN |
...
**Note**: Best data-driven 3-param set is {SEI, D_p, R_mult} (R²=0.985), NOT the Fisher ID group. Fisher-guided set achieves R²=0.969.
